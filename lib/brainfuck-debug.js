    ...
    getInterpreter: function (editor) {
      ...
      bf.on('step', function (output) {
        ...
        if (output) {
          if ((output & 0370) === 0360) {
            outputBufferCount = 4
            outputBuffer = new Buffer(outputBufferCount)
            outputBufferPointer = 0
          } else if ((output & 0360) === 0340) {
            outputBufferCount = 3
            outputBuffer = new Buffer(outputBufferCount)
            outputBufferPointer = 0
          } else if ((output & 0340) === 0300) {
            outputBufferCount = 2
            outputBuffer = new Buffer(outputBufferCount)
            outputBufferPointer = 0
          } else if ((output & 0300) === 0200) {
            
          }
          var bytes = (output & 0360)
          switch (bytes) {
            case 0360:
